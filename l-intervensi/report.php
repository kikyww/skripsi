<?php 
    ...
    $query = mysqli_query($konek, "SELECT * FROM tb_intervensi LEFT JOIN tb_jenisinv ON tb_intervensi.jenis_id = tb_jenisinv.id_jenis LEFT JOIN tb_opd ON tb_intervensi.opd_id = tb_opd.id_opd LEFT JOIN tb_opd AS o ON tb_intervensi.kunjungan_id = o.id_opd LEFT JOIN tb_kecamatan ON tb_intervensi.kecamatan_id = tb_kecamatan.id_kecamatan LEFT JOIN tb_kelurahan ON tb_intervensi.kelurahan_id = tb_kelurahan.id_kelurahan WHERE tb_intervensi.status_intervensi = 'Selesai' ORDER BY tb_intervensi.id_intervensi DESC");
?>
                    ...
                    $('#csv-start').click(function(event) {
                        ...
                        xhr.onload = function() {
                            if (this.status === 200) {
                                ...
                                link.download = 'catatan-intervensi.csv'
                                link.click()
                            }
                        }
                        xhr.send('start_date=' + start_date + '&end_date=' + end_date + '&kecamatan_id=' + filKecamatan + '&kelurahan_id=' + filKelurahan)
                    })
                    ...
                    $('#excel-start').click(function(event) {
                        ...
                        xhr.onload = function() {
                            if (this.status === 200) {
                                ...
                                var link = document.createElement('a')
                                link.href = window.URL.createObjectURL(blob)
                                link.download = 'catatan-intervensi.xlsx'
                                link.click()
                            }
                        }
                        xhr.send('start_date=' + start_date + '&end_date=' + end_date + '&kecamatan_id=' + filKecamatan + '&kelurahan_id=' + filKelurahan)
